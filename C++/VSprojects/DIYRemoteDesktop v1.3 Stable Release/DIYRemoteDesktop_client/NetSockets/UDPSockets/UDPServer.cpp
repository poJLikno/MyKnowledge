...
{	
	inet_ntop(AF_INET, &clientInfo.sin_addr, buffer, (int)size);
}

void UDPServer::SendData(const char *data, size_t size)
{
	int sendOk = sendto(server, data, (int)size, 0, (sockaddr *)&clientInfo, sizeof(clientInfo));
...
int UDPServer::ReceiveData(char *buffer, size_t size)
{
	...
	int bytesIn = recvfrom(server, buffer, (int)size, 0, (sockaddr *)&clientInfo, &clientLength);
